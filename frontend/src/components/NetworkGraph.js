    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-2 mb-6 border-b border-gray-700">
          ...
          <button onClick={() => setActiveTab('graph')} className={`px-4 py-2 font-medium transition-colors ${activeTab === 'graph' ? 'text-red-500 border-b-2 border-red-500' : 'text-gray-400 hover:text-gray-200'}`}>
        ...
        {activeTab === 'graph' && (
          <div>
            <div className="bg-gray-800 rounded-lg p-4 mb-4">
              <h2 className="text-xl font-semibold mb-2">Network Topology Graph</h2>
              <p className="text-sm text-gray-400">
                {attackPaths.length > 0 
                  ? '🔴 Red edges show the highest probability attack path. Drag nodes to rearrange.' 
                  : 'Gray edges show possible network connections. Generate attack paths to see optimal routes highlighted.'}
              </p>
            </div>
            <NetworkGraph networkData={networkData} attackPaths={attackPaths} />
          </div>
        )}
              ...
              <div className="space-y-4">
                <div className="bg-gray-800 rounded-lg p-4 mb-6 flex justify-between items-center">
                  ...
                  <button onClick={exportToPDF} disabled={isExporting} className="bg-green-600 hover:bg-green-700 disabled:bg-gray-600 px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2">
                    {isExporting ? <><div className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent"></div>Generating...</> : <><Download size={18} />Export PDF Report</>}
                  </button>
                </div>
